...
def _delexicalize_all_literals(schema: dict, counter: list, mapping: dict,
                               report: list, path: str = "") -> int:
    if not isinstance(schema, dict):
        return 0
    replaced = 0
    ...
    if "const" in schema and isinstance(schema["const"], str):
        ...
        report.append({
            ...
            "original": schema["const"], "replacement": placeholder,
            "reasons": ["delex_all"],
        })
        schema["const"] = placeholder
        counter[0] += 1
    ...
    if "enum" in schema and isinstance(schema["enum"], list):
        for i, val in enumerate(schema["enum"]):
            ...
            mapping[placeholder] = val
            report.append({
                "path": f"{path}[enum:{i}]", "source": "enum",
                "original": val, "replacement": placeholder,
                "reasons": ["delex_all"],
            })
            schema["enum"][i] = placeholder
            counter[0] += 1
    ...
    for key in ("properties", "patternProperties"):
        if key in schema and isinstance(schema[key], dict):
            for prop_name, prop_schema in schema[key].items():
                replaced += _delexicalize_all_literals(
                    prop_schema, counter, mapping, report, f"{path}.{prop_name}")

    if "items" in schema:
        items = schema["items"]
        if isinstance(items, dict):
            replaced += _delexicalize_all_literals(items, counter, mapping, report, f"{path}[]")
        elif isinstance(items, list):
            for i, item in enumerate(items):
                replaced += _delexicalize_all_literals(item, counter, mapping, report, f"{path}[{i}]")
    ...
    for kw in ("allOf", "anyOf", "oneOf"):
        if kw in schema and isinstance(schema[kw], list):
            for i, sub in enumerate(schema[kw]):
                replaced += _delexicalize_all_literals(sub, counter, mapping, report, f"{path}/{kw}[{i}]")

    if "not" in schema and isinstance(schema["not"], dict):
        replaced += _delexicalize_all_literals(schema["not"], counter, mapping, report, f"{path}/not")

    for kw in ("if", "then", "else"):
        if kw in schema and isinstance(schema[kw], dict):
            replaced += _delexicalize_all_literals(schema[kw], counter, mapping, report, f"{path}/{kw}")
    ...
    if "$defs" in schema and isinstance(schema["$defs"], dict):
        for name, defn in schema["$defs"].items():
            replaced += _delexicalize_all_literals(defn, counter, mapping, report, f"{path}/$defs/{name}")
    if "definitions" in schema and isinstance(schema["definitions"], dict):
        for name, defn in schema["definitions"].items():
            replaced += _delexicalize_all_literals(defn, counter, mapping, report, f"{path}/definitions/{name}")

    return replaced


VALID_DELEX_MODES = ("strip_only", "delex_all", "heuristic_only")


def delexicalize_schema_ablation(response_format: dict,
                                 delex_mode: str = "heuristic_only") -> Tuple[dict, dict, list]:
    if delex_mode not in VALID_DELEX_MODES:
        raise ValueError(f"Invalid delex_mode '{delex_mode}', must be one of {VALID_DELEX_MODES}")
    ...
    if delex_mode == "strip_only":
        pass
    elif delex_mode == "delex_all":
        _delexicalize_all_literals(schema, counter, mapping, report, "")
    elif delex_mode == "heuristic_only":
        _delexicalize_literals(schema, counter, mapping, report, "")

    if stripped_count > 0:
        report.insert(0, {"action": "strip_free_text", "fields_stripped": stripped_count})
