# Utility evaluation of DeLex-JSON defense on JSONSchemaBench.
# For each schema in GlaiveAI-2K + Github_easy: apply DeLex-JSON transform,
# generate via vLLM with sanitized schema, measure JSON validity + schema compliance.
# Validates against the ORIGINAL schema (not sanitized) for compliance.
...
def run_utility_eval(
    ...
    for subset in subsets:
            ...
            for attempt in range(3):
                try:
                    ...
                    if endpoint and not server_alive(endpoint):
                        print(f"  SERVER DEAD detected at schema {idx+1}, exiting", flush=True)
                        server_dead = True
                        break
                    if attempt < 2:
                        time.sleep(2)
        ...
        print(f"  {subset}: {non_error} schemas evaluated (errors={error_count})")
        print(f"  JSON valid: {json_valid_count}/{non_error} ({validity_rate*100:.1f}%)")
        print(f"  Schema compliant: {schema_compliant_count}/{non_error} ({compliance_rate*100:.1f}%)")
        print(f"  Schema modified by DeLex: {modified_count}/{non_error} ({mod_rate*100:.1f}%)")
    ...
    print(f"\n=== Overall Utility (DeLex-JSON) ===")
    print(f"JSON valid: {grand_json_valid}/{grand_total} ({grand_json_valid/grand_total*100:.1f}%)" if grand_total else "")
    print(f"Schema compliant: {grand_schema_compliant}/{grand_total} ({grand_schema_compliant/grand_total*100:.1f}%)" if grand_total else "")
    print(f"Modified: {grand_modified}/{grand_total} ({grand_modified/grand_total*100:.1f}%)" if grand_total else "")
    print(f"New processed this batch: {new_processed}")

    overall["hit_batch_limit"] = hit_batch_limit
    return overall
...
def main():
    ...
    parser.add_argument("--batch-limit", type=int, default=0,
                        help="Max new schemas to process before exiting (0=unlimited). Exit code 2 if limit hit.")
