# Evaluate input guard experiment outputs with guard-refused handling.
# Guard-refused records -> safe (label="no", score=0, no classifier call needed).
# Non-refused records -> classify via HarmBench classifier as normal.
...
def evaluate_with_guard(
    ...
    guard_rejection_rate = refused_count / total if total else 0.0
    ...
    per_category = {
        cat: {
            ...
            "unsafe": v["unsafe"],
            "refused": v["refused"],
            "rejection_rate": v["refused"] / v["total"] if v["total"] else 0,
        }
    ...
    print(f"Guard rejection rate: {guard_rejection_rate*100:.1f}% ({refused_count}/{total})")
...
def main():
    ...
    hb_results = evaluate_with_guard(
    ...
    sr_results = evaluate_with_guard(
